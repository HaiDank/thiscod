...
async function handleNavigateToConversation(friend: User) {
    try {
        const res = await $fetch(`/api/conversations/user/${friend.id}`);
        if (!res) {
            const created = await $csrfFetch(`/api/conversations/user/${friend.id}`, {
                method: "POST",
            });
            if (created) {
